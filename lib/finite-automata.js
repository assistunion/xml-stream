...
FiniteAutomata.prototype._run = function(state, arg) {
  for (var cb in state) {
    if (state.hasOwnProperty(cb)) {
      if (this._callbacks.hasOwnProperty(cb)) {
        this._callbacks[cb](arg);
      }
    }
  }
  return this;
...
FiniteAutomata.prototype.setState = function(state, arg) {
  this._state = state;
  this._run(state, arg);
  return this;
}
...
FiniteAutomata.prototype.go = function(symbol, arg) {
  var next = this.nextState(symbol)
  this.setState(next, arg);
  return this;
};

FiniteAutomata.prototype.pop = function(arg) {
  this._stack[this._stackPtr] = undefined;
  this._run(this._state, arg);
