...
function run(state, args, cbs) {
  for (var cb in state) if (state.hasOwnProperty(cb)) {
...
FiniteAutomata.prototype.onPush = function(state, cb) {
  if (!this._pushCallbacks.hasOwnProperty(state)) {
    this._pushCallbacks[state] = [];
  }
  this._pushCallbacks[state].push(cb);
  return this;
}

FiniteAutomata.prototype.onPop = function(state, cb) {
  if (!this._popCallbacks.hasOwnProperty(state)) {
    this._popCallbacks[state] = [];
  }
  this._popCallbacks[state].push(cb);
  return this;
}

FiniteAutomata.prototype.on = FiniteAutomata.prototype.onPop;

FiniteAutomata.prototype.setState = function(state, args) {
  this._state = state;
  run(state, args, this._popCallbacks);
  return this;
}
...
FiniteAutomata.prototype.pop = function(args) {
  ...
  run(this._state, args, this._popCallbacks);
  this._state = this._stack[--this._stackPtr];
  return this;
};

FiniteAutomata.prototype.push = function(symbol, args) {
  if (args == null) {
    args = [];
  }
  var next = this.nextState(symbol);
  ...
  this._state = next;
  run(this._state, args, this._pushCallbacks);
  return this;
};
