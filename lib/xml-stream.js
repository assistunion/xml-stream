    ...
    if (event.type === 'updateElement') {
      ...
      this._fa.on('leave', finalState, function(element, context, trace) {
        if (!--self._bufferLevel) {
          ...
          if (self._emitData) {
              emitElement.call(self, element, node.name, true);
          }
        }
      });
    } else {
      var fn = function(node, context, trace) {
        self.emit(event.name, node, context, trace);
      };
...
function parse(stream) {
  ...
  xml.on('text', function(text) {
    ...
    if (self._bufferLevel === 0 && self._emitData) {
      emitText.call(self, curr.element.text);
    }
    curr.fullText += curr.element.$text.trim();
  });
