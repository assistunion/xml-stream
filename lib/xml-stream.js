...
function parse(stream) {
  ...
  xml.on('startElement', function(name, attr) {
    ...
    context[name] = curr.element;
    curr.context = context;
    fa.enter(name, [curr.element, curr.context, trace]);
    if (isEmpty(curr.element.$)) {
      delete curr.element.$;
    }
  });
  ...
  xml.on('endElement', function(name) {
    ...
    if (isEmpty(curr.element)) {
      val  = curr.text;
    } else if (isEmpty(curr.element)) {
      val = new String(curr.text);
    } else if (curr.text !== '') {
      val = new String(curr.text);
      for(var x in curr.element) if (curr.element.hasOwnProperty(x)) {
        val[x] = curr.element[x];
      }
    } else {
      val = curr.element;
    }
    curr.context[name] = val;
    prev.element[curr.name] = val;
    fa.leave([val, curr.context, trace]);
    curr = prev;
  });
  ...
  xml.on('text', function(text) {
    ...
    fa.run([textWrapper, curr.context, trace]);
    curr.text += textWrapper.text.trim();
  });
