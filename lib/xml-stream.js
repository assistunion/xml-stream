...
//
// Supported selector events:
// * `match[: selector]` is emitted for each occurence of any node, or
//   for each node matching the selector.
//
...
XmlStream.prototype.on = function(event, listener) {
  ...
  if (m === null) {
    // Delegate to original EventEmitter.on, if we're not dealing with
    // selectors. Otherwise, invoke selector-specific processing logic.
...
XmlStream.prototype.parse = function(source, cb) {
  ...
  // End parsing on stream EOF and run a callback.
